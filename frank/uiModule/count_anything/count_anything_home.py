...
from PyQt5.QtCore import QSize, QTimer, Qt, QEvent, QRect
from PyQt5.QtGui import QFont, QMovie, QPainter, QPen
...
from frank.fieldModule.all_fields import CountAnythingFields as FIELDS

from qfluentwidgets import FluentIcon as FIF

class CountAnythingHome(QWidget):
    def __init__(self):
        super().__init__()
        self.setup_ui()
        self.drawing = False
        ...
        self.widget_display.installEventFilter(self)
    ...
    def setup_ui(self):
        ...
        # self.workspace_widget.setStyleSheet("background: white;")
        self.workspace_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)

        self.left_layout.addWidget(self.workspace_widget)

        # 左侧工具栏
        self.tool_list = QWidget()
        self.tool_list_layout = QVBoxLayout(self.tool_list)
        self.tool_list_layout.setContentsMargins(0, 10, 0, 0)
        self.tool_list_layout.setAlignment(Qt.AlignTop)
        self.tool_list_layout.setSpacing(10)
        self.workspace_layout.addWidget(self.tool_list)

        # btn
        self.btn_rect_choose = ToggleToolButton(FIF.TRANSPARENT)
        self.tool_list_layout.addWidget(self.btn_rect_choose)
        self.btn_clear = ToolButton(FIF.DELETE)
        self.tool_list_layout.addWidget(self.btn_clear)
        ...
        self.workspace_layout.addWidget(self.background)

        # 因为要保持正方形，所以必须写成子组件，不能让布局管理器管理
        self.widget_display = DrawRectLabel(self.background)
