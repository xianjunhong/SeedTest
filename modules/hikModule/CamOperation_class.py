...
class CameraOperation:
    ...
    def open_device(self):
        if not self.b_open_device:
            ...
            # ch:设置触发模式为off | en:Set trigger model as off
            ret = self.obj_cam.MV_CC_SetEnumValue("TriggerMode", MV_TRIGGER_MODE_OFF)
            if ret != 0:
                print("set trigger model fail! ret[0x%x]" % ret)
            return MV_OK
    # 设置 ROI
    def set_roi(self, offset_x, offset_y, width, height,reverse_x,Reverse_y):
        print("进入roi")
        if not self.b_open_device:
            return MV_E_CALLORDER

        self.obj_cam.MV_CC_SetIntValue("OffsetX", 0)
        self.obj_cam.MV_CC_SetIntValue("OffsetY", 0)
        ...
        # 设置镜像
        ret = self.obj_cam.MV_CC_SetBoolValue("ReverseX", reverse_x);
        if ret != 0:
            return ret
        ret = self.obj_cam.MV_CC_SetBoolValue("ReverseY", Reverse_y);
        if ret != 0:
            return ret
        ...
        ret = self.obj_cam.MV_CC_SetIntValue("OffsetX", offset_x)
        if ret != 0:
            return ret


        ret = self.obj_cam.MV_CC_SetIntValue("OffsetY", offset_y)
        if ret != 0:
            return ret
    ...
    def Save_jpg(self, ImageFolder,uid):
        if self.buf_save_image is None:
            return None, "Buffer save image is None", None
        ...
        # **格式化文件名**
        file_path = os.path.join(ImageFolder, file_name)  # 绝对路径
